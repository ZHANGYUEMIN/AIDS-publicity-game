...
function addStudentDataToClass(studentName, score, answers) {
    ...
    // 尝试保存到localStorage和sessionStorage
    try {
        localStorage.setItem('classData', JSON.stringify(classData));
        console.log('班级数据已成功保存到localStorage (学生数据添加)');
        
        // 同时保存到sessionStorage作为备份
        try {
            sessionStorage.setItem('classData', JSON.stringify(classData));
        } catch (sessionError) {
            console.error('保存班级数据到sessionStorage失败:', sessionError);
        }
    } catch (error) {
        console.error('保存班级数据到localStorage时出错:', error);
        // 尝试使用备用方法：仅使用sessionStorage
        try {
            sessionStorage.setItem('classData', JSON.stringify(classData));
            console.log('班级数据已保存到sessionStorage (备用方案)');
        } catch (fallbackError) {
